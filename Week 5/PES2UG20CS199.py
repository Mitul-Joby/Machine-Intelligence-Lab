...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        if gradients is None:
            if grad1.requires_grad: 
                grad1.grad += np.ones_like(grad1.arr)
            if grad2.requires_grad: 
                grad2.grad += np.ones_like(grad2.arr)
        else:
            if grad1.requires_grad: 
                grad1.grad = np.ones_like(grad1.arr) * gradients
            if grad2.requires_grad: 
                grad2.grad = np.ones_like(grad2.arr) * gradients
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients is None:
            if grad1.requires_grad:
                grad1.grad += (np.ones_like(grad1.arr) @ grad2.arr.transpose())
            if grad2.requires_grad:
                grad2.grad += (np.ones_like(grad2.arr) @ grad1.arr).transpose()
        else:
            if grad1.requires_grad:
                grad1.grad += (np.ones_like(grad1.arr) @ grad2.arr.transpose()) * gradients
            if grad2.requires_grad:
                grad2.grad += (np.ones_like(grad2.arr) @ grad1.arr).transpose() * gradients
    ...
    def backward(self, gradients=None):
        ...
        if self.history[0] == 'add' or self.history[0] == 'matmul':
            grad1, grad2 = self.grad_add(gradients) if self.history[0] == 'add' else self.grad_matmul(gradients)        
            if self.history[1]:

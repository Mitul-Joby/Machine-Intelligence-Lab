...
class KMeansClustering:
    ...
    def e_step(self, data):
        ...
        for i, x in enumerate(data):
            cluster_assign[i] = np.argmin(np.sum((x - self.centroids)**2, axis=1))
    ...
    def m_step(self, data, cluster_assgn):
        ...
        for i in range(self.n_cluster):
            self.centroids[i] = data[cluster_assgn == i].mean(axis=0)
    ...
    def evaluate(self, data):
        ...
        for M in range(data.shape[0]):
            for D in range(self.n_cluster):
                metric += np.sum((data[M] - self.centroids[D])**2)
        return metric
